    ...
    <div id="wrapper">
        ...
        <h2>Credit Liability
            <button class="btn btn-sm btn-default">
                 <a href="{{url('gen/C_liabilityExcel')}}">Gen Excel</a>
            </button>
        </h2>
        <!--<button style="display:none;"class="btn btn-info btn-block" id="credit-form-btn">Hide Create Liability Form</button>   
                    ...
                    <form class="form-horizontal" role="form" method="POST" action="{{ url('creditliability') }}">
                        <input id="token"type="hidden" name="_token" value="{{ csrf_token() }}">
                            ...
                            <label class="control-label">TrackingNo</label>
                                ...
                                <input type="text"placeholder="Principal"  id="principal"class="form-control" name="principal" value="{{ old('principal') }}"></br>

                                <input type="text"placeholder="Intrest"  id="intrest"class="form-control" name="intrest" value="{{ old('intrest') }}"></br>

                                <input type="text"placeholder="Installment"  id="installment"class="form-control" name="installment" value="{{ old('perInstallment') }}"></br>
                            ...
                            <label class="control-label">Confirmation Code(M-pesa,Bank Payments)</label>
                                ...
                                <input type="text"placeholder="Creditor"  id="creditor"class="form-control" name="creditor" value="{{ old('creditor') }}"></br>

                                <input type="text"placeholder="Email"  id="email"class="form-control" name="email" value="{{ old('email') }}"></br>

                                <input type="text"id="phone"placeholder="PhoneNo" class="form-control" name="phone" value="{{ old('phone') }}"></br>
         ...
         <!--END OF CREDIT-FOMR-->
        <!--<button class="btn btn-info btn-block" id="view-credit-btn">Create Credit Liability Entry</button>-->
          <div id="view-credit" class="row">
           <div id="view-credit-header"class="col-sm-12">
             <div class="col-sm-1">Creditor</div>
              <div class="col-sm-2">TrackingNo</div>
              <div class="col-sm-2">Type</div>
              <div class="col-sm-2">Description</div>
              <div class="col-sm-2">Principal</div>
              <div class="col-sm-1">Intrest</div>
              <div class="col-sm-2">Balance</div>
           </div>
           @foreach($creditliabilities as $creditliability)
           <div id="view-credit-details" class="col-sm-12">
              <div class="col-sm-1">{{$creditliability->creditor}}</div>
              <div class="col-sm-2">{{$creditliability->trackingNo}}</div>
              <div class="col-sm-2">{{$creditliability->type}}</div>
              <div class="col-sm-2">{{$creditliability->description}}</div>
              <div class="col-sm-2">
                  <button class="btn btn-sm btn-info">KSH{{$creditliability->principal}}</button>
               </div>
              <div class="col-sm-1">
                  <button class="btn btn-sm btn-primary">{{$creditliability->intrest}}</button>
              </div>
              <div class="col-sm-2">
                <button class="btn btn-sm btn-warning">KSH{{$creditliability->balance}}</button>
            </div>
           </div>
           @endforeach
           <?php echo $creditliabilities->render()?>
         </div>
        </div>
        <!-- /#page-wrapper -->
